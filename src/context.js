...
class ContextProvider extends Component {
  ...
  componentDidMount() {
    ...
    let filterdRooms = rooms.filter((obj) => obj.featured === true);
    this.setState({
      allRoom: rooms,
      featuredRooms:filterdRooms,
      defienRomms: rooms,
      loading: false,
    });
  }
  ...
  HfrezHate = (obj) => {
    let firstlist = obj.map((ob) => {
      ...
      let imagelist = ob.fields.images.map((img) => img.fields.file.url);
    ...
    return (
      <firstContext.Provider
        value={{
          ...this.state,
          hepy: this.getRoom,
          maleFunction: this.maleFunction,
